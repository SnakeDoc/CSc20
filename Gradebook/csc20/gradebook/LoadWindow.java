...
public class LoadWindow implements ActionListener {
    ...
    private JPanel getHeader() {
        ...
        JLabel headerLabel = new JLabel("                         Load Students From A File");
	...
	public void actionPerformed(final ActionEvent e) {
		if (e.getActionCommand().equals("Load")) {
			if (!"".equals(classText.getText()) || classText.getText() != null) {
				try {
					FileInputStream fis = new FileInputStream(classText.getText());
					ObjectInputStream ois = new ObjectInputStream(fis);
					Main.setAllStudents((List<Student>) ois.readObject());
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
